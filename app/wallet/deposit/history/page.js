...
export default function DepositHistoryPage() {
  ...
  if (!mounted) {
    return (
      <main style={{ background: "#fafafa", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", color: "#222222" }}>
        <div>Loading...</div>
      </main>
    );
  }
  ...
  return (
    <main style={{ background: "#fafafa", minHeight: "100vh", color: "#222222", padding: "1.5rem 1rem", fontFamily: "sans-serif", maxWidth: "480px", margin: "0 auto", position: "relative" }}>
      ...
      <header style={{ 
        ...
        <Link href="/wallet/deposit" style={{ color: "#ffffff", textDecoration: "none", fontSize: "2rem", padding: "0 0.5rem" }} aria-label="Back">
          ‹
        </Link>
        <h1 style={{ fontSize: "1.125rem", fontWeight: "600", margin: 0 }}>Deposit history</h1>
      ...
      <div style={{ display: "flex", gap: "10px", overflowX: "auto", paddingBottom: "0.75rem", marginBottom: "1rem" }}>
        <button onClick={() => setMethodFilter("all")} style={{
          background: methodFilter === "all" ? "#009688" : "#ffffff",
          color: methodFilter === "all" ? "#ffffff" : "#4e4e4e",
          border: methodFilter === "all" ? "none" : "1px solid #eaeaea",
          borderRadius: "8px",
          padding: "0.5rem 1rem",
          fontSize: "0.85rem",
          fontWeight: "bold",
        ...
        <button onClick={() => setMethodFilter("trc20")} style={{
          background: methodFilter === "trc20" ? "#009688" : "#ffffff",
          color: methodFilter === "trc20" ? "#ffffff" : "#4e4e4e",
          border: methodFilter === "trc20" ? "none" : "1px solid #eaeaea",
          borderRadius: "8px",
          padding: "0.5rem 1rem",
          fontSize: "0.85rem",
          fontWeight: "bold",
        ...
        <button onClick={() => setMethodFilter("bep20")} style={{
          background: methodFilter === "bep20" ? "#009688" : "#ffffff",
          color: methodFilter === "bep20" ? "#ffffff" : "#4e4e4e",
          border: methodFilter === "bep20" ? "none" : "1px solid #eaeaea",
          borderRadius: "8px",
          padding: "0.5rem 1rem",
          fontSize: "0.85rem",
          fontWeight: "bold",
      ...
      <div style={{ display: "flex", gap: "10px", marginBottom: "1.5rem" }}>
        <select
          ...
          style={{ flex: 1, background: "#ffffff", border: "1px solid #eaeaea", borderRadius: "8px", color: "#222222", padding: "0.75rem", outline: "none", fontSize: "0.85rem" }}
      ...
      {loading && deposits.length === 0 ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
          {filtered.map((d) => {
            ...
            return (
              <div key={d._id} style={{ background: "#ffffff", border: "1px solid #eaeaea", borderRadius: "14px", padding: "1.25rem", display: "flex", flexDirection: "column", gap: "10px", boxShadow: "0 2px 8px rgba(0,0,0,0.05)" }}>
                
                {/* TOP HEADER */}
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <span style={{ background: "rgba(0, 150, 136, 0.1)", color: "#009688", border: "1px solid rgba(0, 150, 136, 0.2)", padding: "3px 9px", borderRadius: "6px", fontSize: "0.75rem", fontWeight: "bold" }}>
                    Deposit
                  </span>
                  <span style={{
                ...
                {/* DETAILS ROWS */}
                <div style={{ display: "flex", flexDirection: "column", gap: "9px", borderTop: "1px solid #eaeaea", paddingTop: "9px", marginTop: "2px" }}>
                  
                  {/* BALANCE */}
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.88rem" }}>
                    <span style={{ color: "#666666" }}>Balance</span>
                    <span style={{ color: "#009688", fontWeight: "bold" }}>₹{formatAmount(d.amount)}</span>
                  </div>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.88rem" }}>
                    ...
                    <span style={{ color: "#222222" }}>{new Date(d.createdAt).toLocaleString("en-IN")}</span>
                  </div>

                  {/* ORDER NUMBER */}
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.88rem" }}>
                    <span style={{ color: "#666666" }}>Order number</span>
                    <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                      <span style={{ color: "#4e4e4e", fontFamily: "monospace", fontSize: "0.82rem" }}>{orderNo}</span>
                  ...
                  {rejectReason && (
                    <div style={{ display: "flex", flexDirection: "column", fontSize: "0.82rem", marginTop: "4px", padding: "6px 10px", background: "rgba(239, 68, 68, 0.08)", borderLeft: "3px solid #ef4444", borderRadius: "4px", gap: "2px" }}>
                      ...
                      <span style={{ color: "#222222" }}>{rejectReason}</span>
                    </div>
                  )}
